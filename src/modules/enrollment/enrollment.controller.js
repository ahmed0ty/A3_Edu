  ...
  async (req, res, next) => {
    try {
      await cache(() => myCoursesKey(req.user._id))(
        ...
        res,
        async () => {
          const courses = await enrollmentService.getStudentCourses(req.user._id);
...
export const updatePaymentStatus = async (req, res, next) => {
  try {
    ...
    const updatedEnrollment = await enrollmentService.updatePaymentStatus(
      enrollmentId,
      {
        status,
        transactionId,
...
export const createPaymentSession = async (req, res, next) => {
  try {
    ...
    const paymentUrl = `https://ipn.eg/S/chillout.egypt/instapay/${enrollmentId}`;
